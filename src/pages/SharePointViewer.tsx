...
const SharePointViewer: React.FC = () => {
  ...
  const visibleClientSummaries = useMemo(() => {
    const normalizedSearch = normalizeString(clientSearch);
    if (!normalizedSearch) {
      return clientSummaries;
    }
    return clientSummaries.filter(summary => normalizeString(summary.client).includes(normalizedSearch));
  ...
  const filteredWorkOrders = useMemo(() => {
    const normalizedSearch = normalizeString(workOrderSearch);
    return workOrderEntries.filter(entry => {
      if (selectedClientFilter !== 'all' && entry.client !== selectedClientFilter) {
        return false;
      }
      if (normalizedSearch && !normalizeString(entry.workOrderNumber).includes(normalizedSearch)) {
        return false;
      }
      return true;
    });
  }, [selectedClientFilter, workOrderEntries, workOrderSearch]);
  ...
  const filteredBatches = useMemo(() => {
    const normalizedSearch = normalizeString(batchSearch);
    return batchEntries.filter(entry => {
      if (selectedClientFilter !== 'all' && entry.client !== selectedClientFilter) {
        return false;
      }
      if (batchStatusFilter !== 'all' && entry.statusKey !== batchStatusFilter) {
        return false;
      }
      if (normalizedSearch && !normalizeString(entry.batchNumber).includes(normalizedSearch)) {
        return false;
      }
      return true;
    });
  }, [batchEntries, batchStatusFilter, batchSearch, selectedClientFilter]);
        ...
        <TabsContent value="clients" className="space-y-6">
          <Card className="border-2 shadow-lg">
            ...
            <CardContent className="space-y-4">
              ...
              {clientSummaries.length === 0 ? (
              ...
              ) : visibleClientSummaries.length > 0 ? (
                ...
                <div className="text-center py-10 text-muted-foreground">No clients match your search.</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:flex-wrap md:items-end md:justify-between">
                ...
                <div className="space-y-2 md:text-right">
                  ...
                  <Input
                    ...
                    className="w-full border-2 md:w-[220px]"
                  />
                </div>
              </div>
